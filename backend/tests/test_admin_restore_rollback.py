...
TEST_DB_FILE = "./test_admin_restore_rollback.db"
...
def _failed_event_count(path: Path, backup_id: int):
    con = sqlite3.connect(path)
    cur = con.cursor()
    cur.execute("select count(*) from admin_backup_restore_events where backup_id=? and status in ('failed','rolled_back')", (backup_id,))
    out = cur.fetchone()[0]
    con.close()
    return out


def test_restore_rolls_back_when_swap_fails(db, monkeypatch):
    admin = db.query(User).filter(User.email == "admin.restore.rollback@example.com").first()
    if not admin:
        admin = User(
            email="admin.restore.rollback@example.com",
            hashed_password="x",
            full_name="Admin Restore Rollback",
    ...
    backup_row = backup_service.create_manual_backup(db=db, org_id=admin.organization_id, user_id=admin.id)
    ...
    def fail_during_swap(src, dst):
        state["count"] += 1
        if state["count"] == 2:
            raise RuntimeError("forced_swap_failure")
        return original_replace(src, dst)

    monkeypatch.setattr(backup_service.os, "replace", fail_during_swap)
    ...
    # Restore failure must preserve pre-existing active data.
    assert sorted(_sentinel_values(db_path)) == ["after", "before"]

    assert _failed_event_count(db_path, backup_id) >= 1
    assert (
