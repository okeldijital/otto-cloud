...
async def scc_db_inventory(current_user: User = Depends(get_current_admin_user)):
    ...
    files = inventory_sqlite_files(app_data_dir=app_data_dir, current_database_url=settings.DATABASE_URL)
    return {
...
async def scc_db_switch(
    ...
    candidate = Path(payload.sqlite_path).expanduser().resolve()
    if not candidate.exists() or not candidate.is_file():
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail="sqlite_path not found",
        )

    if candidate.suffix.lower() not in {".sqlite", ".db"}:
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail="sqlite_path must be a .sqlite or .db file",
        )

    if not sqlite_integrity_ok(candidate):
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail="sqlite integrity check failed",
        )

    pointer_payload = write_active_db_pointer(sqlite_path=candidate, updated_by=current_user.email)
    return {
        "status": "switched",
        "database_url": pointer_payload["database_url"],
        "pointer_file": settings.ACTIVE_DB_POINTER_FILE,
