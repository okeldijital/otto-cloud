...
import { activityOrgScopeWhere, requireActorUserId, requireLegacyIntOrgId, royaltyOrgScopeWhere } from "@/lib/auth/resource-authorization";
...
const definitions: ReportDefinition[] = [
  {
    ...
    async run(ctx) {
      ...
      return { columns: [{ key: "entity", label: "Entity" }, { key: "count", label: "Count" }], rows: [
        { entity: "Artists", count: artists }, { entity: "Releases", count: releases }, { entity: "Tracks", count: tracks }, { entity: "Works", count: works }, { entity: "Labels", count: labels }, { entity: "Publishers", count: publishers }, { entity: "PROs", count: pros },
  ...
  },
  {
    ...
    async run(ctx) {
      const contracts = await prisma.contracts.findMany({
        where: { organization_id: requireLegacyIntOrgId(ctx) },
        include: { _count: { select: { contract_parties: true, contract_documents: true, contract_assets: true } } },
      });
      ...
      return { columns: [
        { key: "id", label: "ID" }, { key: "title", label: "Title" }, { key: "contract_number", label: "Contract #" }, { key: "status", label: "Status" }, { key: "type", label: "Type" }, { key: "parties", label: "Parties" }, { key: "documents", label: "Documents" }, { key: "assets", label: "Assets" }, { key: "complete", label: "Complete" },
      ], rows, summary: { total: rows.length, status_distribution: statusDist, complete: rows.filter((r) => r.complete).length, incomplete: rows.filter((r) => !r.complete).length, missing_documents: rows.filter((r) => !r.has_documents).length, missing_parties: rows.filter((r) => !r.has_parties).length } };
    },
  },
  {
    ...
    defaultParams: {},
    async run(ctx) {
      const royalties = await prisma.royalties.findMany({ where: royaltyOrgScopeWhere(ctx) });
  ...
  },
  {
    ...
    async run(ctx, params) {
      ...
      const activities = await prisma.activities.findMany({ where: activityOrgScopeWhere(ctx), orderBy: { timestamp: "desc" as const }, take: limit });
...
export function getReportDefinitions(): Pick<ReportDefinition, "type" | "name" | "description" | "defaultParams">[] {
  return definitions.map((d) => ({ type: d.type, name: d.name, description: d.description, defaultParams: d.defaultParams }));
}

export function getReportDefinition(type: string): ReportDefinition | undefined { return definitions.find((d) => d.type === type); }

export async function runReport(ctx: OrganizationContext, reportType: string, params: Record<string, any> = {}): Promise<{ runId: number; result: ReportResult }> {
  ...
  const run = await prisma.report_runs.create({ data: { organization_id: orgId, status: "running", requested_by_user_id: userId, parameters_json: JSON.stringify({ report_type: reportType, ...params }) } });
  try {
