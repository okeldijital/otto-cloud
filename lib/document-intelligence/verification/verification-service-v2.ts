...
export class VerificationServiceV2 extends BaseVerificationService {
  ...
  override async complete(params: Parameters<BaseVerificationService["complete"]>[0]) {
    ...
    if (incompleteRequired.length > 0) {
      throw new IntelligenceError(`Required fields still in draft or unresolved: ${incompleteRequired.join(", ")}`, 400, "REQUIRED_FIELDS_PENDING", incompleteRequired);
    }

    const stillDraft = fields.filter((field) => field.verificationState === FIELD_VERIFICATION_STATE.draft && field.value != null && String(field.value).trim() !== "");
    if (stillDraft.length > 0) {
      throw new IntelligenceError(`Extracted fields still require review (${stillDraft.length})`, 400, "DRAFT_FIELDS_REMAIN", stillDraft.map((field) => field.fieldKey));
    }
      ...
      for (const field of fields) {
        ...
        if (humanNotFound) {
          await tx.verifiedField.create({ data: {
            ...
            fieldKey: field.fieldKey, fieldLabel: field.fieldLabel, verifiedValue: null, decision: "not_found",
            aiValue: sourceLocation.aiOriginalValue ?? field.value, aiConfidence: field.confidence,
