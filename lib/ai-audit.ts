...
import { requireLegacyIntOrgId, royaltyOrgScopeWhere, trackOrgScopeWhere } from "@/lib/auth/resource-authorization";
...
function orgFilter(orgId: string) { return { organization_id: orgId }; }
...
export async function checkCatalogConsistency(ctx: OrganizationContext): Promise<AuditReport> {
  ...
  const orphans = await prisma.tracks.findMany({ where: { release_id: null, ...trackScope }, take: 50 });
  ...
  const tracksWithoutWork = await prisma.tracks.findMany({ where: { work_id: null, ...trackScope }, take: 50 });
  ...
  const releasesWithoutTracks = await prisma.releases.findMany({ where: { ...orgFilter(ctx.organizationId), is_deleted: false }, include: { _count: { select: { track_releases: true } } }, take: 50 });
  ...
  const worksWithoutTracks = await prisma.works.findMany({ where: { ...orgFilter(ctx.organizationId), is_deleted: false }, take: 50 });
  const linkedWorkIds = new Set((await prisma.tracks.findMany({ where: { work_id: { not: null }, ...trackScope }, select: { work_id: true } })).map((t) => t.work_id).filter(Boolean) as number[]);
...
export async function checkReleaseQuality(ctx: OrganizationContext): Promise<AuditReport> {
  const findings: AuditFinding[] = [];
  const releases = await prisma.releases.findMany({ where: { ...orgFilter(ctx.organizationId), is_deleted: false }, take: 100 });
  for (const r of releases) {
    ...
    const artistIds = r.artist_ids as unknown as number[] | null; if (!r.artist_id && !artistIds?.length) missing.push("artist"); if (!r.label_id) missing.push("label");
...
export async function checkRoyaltyAnomalies(ctx: OrganizationContext): Promise<AuditReport> {
  const findings: AuditFinding[] = [];
  const royalties = await prisma.royalties.findMany({ where: royaltyOrgScopeWhere(ctx), take: 500 });
  if (royalties.length > 0) {
    const amounts = royalties.map((r) => r.amount?.toNumber() || 0); const mean = amounts.reduce((s, v) => s + v, 0) / amounts.length; const stdDev = Math.sqrt(amounts.reduce((s, v) => s + (v - mean) ** 2, 0) / amounts.length); const threshold = mean + 3 * stdDev;
    for (const r of royalties) { const amt = r.amount?.toNumber() || 0; if (amt > threshold && amt > 1000) findings.push({ issue_type: "royalty_anomaly", severity: "AMBER", entity_type: "royalty", entity_id: r.id, summary: `Royalty #${r.id} (${r.source || "unknown"}): $${amt.toFixed(2)} is ${(amt / mean).toFixed(1)}x the mean`, details: `Amount $${amt.toFixed(2)} exceeds threshold of $${threshold.toFixed(2)} (mean: $${mean.toFixed(2)}, SD: $${stdDev.toFixed(2)})` }); }
...
export async function checkContracts(ctx: OrganizationContext): Promise<AuditReport> {
  const findings: AuditFinding[] = [];
  const contracts = await prisma.contracts.findMany({ where: { organization_id: requireLegacyIntOrgId(ctx) }, include: { _count: { select: { contract_parties: true, contract_documents: true, contract_assets: true } } }, take: 100 });
...
export async function runAllAudits(ctx: OrganizationContext): Promise<AuditReport[]> { return Promise.all([checkCatalogConsistency(ctx), checkReleaseQuality(ctx), checkRoyaltyAnomalies(ctx), checkContracts(ctx)]); }

export async function postFindingsToStatusQuo(ctx: OrganizationContext, findings: AuditFinding[]): Promise<number> {
  const orgId = ctx.organizationId; let count = 0;
  for (const f of findings) {
    ...
    if (!existing) { await prisma.status_quo_items.create({ data: { organization_id: orgId, entity_type: f.entity_type, entity_id: f.entity_id, issue_type: f.issue_type, severity: f.severity, summary: f.summary, details_json: f.details ? JSON.stringify({ details: f.details }) : null, created_at: new Date() } }); count++; }
  }
  return count;
}
