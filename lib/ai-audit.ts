...
export async function checkContracts(ctx: OrganizationContext): Promise<AuditReport> {
  ...
  for (const c of contracts) {
    if (!c._count.contract_documents) {
      findings.push({ issue_type: "missing_document", severity: "RED", entity_type: "contract", entity_id: c.id, summary: `Contract "${c.title || `#${c.id}`}" has no signed document` });
    }
    if (!c._count.contract_parties) {
      findings.push({ issue_type: "missing_parties", severity: "RED", entity_type: "contract", entity_id: c.id, summary: `Contract "${c.title || `#${c.id}`}" has no parties` });
    }
    if (!c.start_date || !c.end_date) {
      findings.push({ issue_type: "missing_dates", severity: "AMBER", entity_type: "contract", entity_id: c.id, summary: `Contract "${c.title || `#${c.id}`}" is missing effective dates` });
    }
  }
