...
export class MembershipService {
  ...
  async listForIdentity(identityId: string) {
    return membershipRepository.listForIdentity(identityId);
  }
  ...
  async createMembership(params: {
    ...
    const policies = await organizationPolicyService.getPolicies(
      params.organizationId
    );
    const count = await membershipRepository.countActive(params.organizationId);
    if (count >= policies.maxMembers) {
    ...
    const role = await roleRepository.findByKey(
      params.organizationId,
      roleKey
    );
    ...
    await membershipRepository.audit({
      organizationId: params.organizationId,
      membershipId: m.id,
      identityId: params.identityId,
    ...
    effectivePermissionCache.invalidatePrefix(
      params.identityId,
      params.organizationId
    );

    await emitIdentityEvent({
      eventType: IDENTITY_EVENTS.MembershipCreated,
      identityId: params.identityId,
      organizationId: params.organizationId,
      payload: { membershipId: m.id, roleKey },
    });

    await emitIdentityEvent({
      eventType: IDENTITY_EVENTS.RoleAssigned,
      identityId: params.identityId,
      organizationId: params.organizationId,
      payload: { roleKey, membershipId: m.id },
    });
  ...
  async setRole(params: {
    ...
    if (!membership || membership.status === "removed") {
      throw new IdentityError("Membership not found", 404, "MEMBERSHIP_NOT_FOUND");
    }
    const role = await roleRepository.findByKey(
      params.organizationId,
      params.roleKey
    ...
    await membershipRepository.audit({
      ...
      payload: { roleKey: params.roleKey },
    });
    effectivePermissionCache.invalidatePrefix(
      params.identityId,
      params.organizationId
  ...
  async suspend(params: {
    ...
    await emitIdentityEvent({
      eventType: IDENTITY_EVENTS.MembershipSuspended,
      identityId: params.identityId,
      organizationId: params.organizationId,
  ...
  async reactivate(params: {
    ...
    await membershipRepository.updateStatus(membership.id, "active", {
      suspendedAt: null,
    });
    await membershipRepository.audit({
      organizationId: params.organizationId,
    ...
    await emitIdentityEvent({
      eventType: IDENTITY_EVENTS.MembershipReactivated,
      identityId: params.identityId,
      organizationId: params.organizationId,
      payload: { membershipId: membership.id },
    });
  ...
  async remove(params: {
    ...
    await membershipRepository.updateStatus(membership.id, "removed", {
      removedAt: new Date(),
    });
    await membershipRepository.audit({
      organizationId: params.organizationId,
    ...
    await emitIdentityEvent({
      eventType: IDENTITY_EVENTS.MembershipRemoved,
      identityId: params.identityId,
      organizationId: params.organizationId,
      payload: { membershipId: membership.id },
    });
  ...
  async transferOwnership(params: {
    ...
    const newOwner = await membershipRepository.find(
    ...
    if (org.ownerIdentityId) {
      const prev = await membershipRepository.find(
        org.ownerIdentityId,
        params.organizationId
      );
      if (prev) {
        await membershipRepository.upsert({
    ...
    const ownerRole =
      (await roleRepository.findByKey(params.organizationId, "owner")) ||
      (await roleRepository.findByKey(params.organizationId, "org_admin"));

    await organizationRepository.update(params.organizationId, {
      ownerIdentityId: params.newOwnerIdentityId,
    });
    await membershipRepository.upsert({
      identityId: params.newOwnerIdentityId,
      organizationId: params.organizationId,
      roleId: ownerRole?.id ?? newOwner.roleId,
      isOwner: true,
      status: "active",
    });
    ...
    await membershipRepository.audit({
      ...
      payload: { previousOwnerId: org.ownerIdentityId },
    });

    effectivePermissionCache.invalidatePrefix(params.newOwnerIdentityId);
    if (org.ownerIdentityId) {
      effectivePermissionCache.invalidatePrefix(org.ownerIdentityId);
    }
  }
}
