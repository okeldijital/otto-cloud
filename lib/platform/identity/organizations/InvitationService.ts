...
import {
  ...
  hashToken,
  normalizeEmail,
} from "../authentication/crypto/tokens";
...
export class InvitationService {
    ...
    const withRoles = await Promise.all(
      rows.map(async (r) => {
        ...
        if (r.roleId) {
          ...
          roleKey = roles.find((x) => x.id === r.roleId)?.key ?? null;
        }
        return {
          id: r.id,
  ...
  async create(params: {
    ...
    roleKey?: string;
    invitedById: string;
  }): Promise<{ invitation: InvitationDto; inviteUrl: string; token: string }> {
    ...
    const expiresAt = new Date(
      Date.now() + policies.invitationTtlDays * 24 * 60 * 60 * 1000
    );
    ...
    if (process.env.NODE_ENV !== "production") {
      console.info(`[iam] invitation for ${email}: ${inviteUrl}`);
    }

    await emitIdentityEvent({
      eventType: IDENTITY_EVENTS.InvitationCreated,
      identityId: params.invitedById,
      organizationId: params.organizationId,
      payload: { invitationId: inv.id, email },
    });
    await emitIdentityEvent({
      eventType: IDENTITY_EVENTS.InvitationSent,
      identityId: params.invitedById,
      organizationId: params.organizationId,
      payload: { invitationId: inv.id, email },
    });
  ...
  async cancel(params: {
    ...
    if (!inv) {
      throw new IdentityError("Invitation not found", 404, "INVITATION_NOT_FOUND");
    }
    ...
    await emitIdentityEvent({
      eventType: IDENTITY_EVENTS.InvitationCancelled,
  ...
  async accept(params: {
    ...
    password?: string;
    displayName?: string;
  }): Promise<{ identityId: string; organizationId: string }> {
    ...
    if (inv.expiresAt <= new Date()) {
      ...
      await emitIdentityEvent({
        eventType: IDENTITY_EVENTS.InvitationExpired,
        organizationId: inv.organizationId,
        payload: { invitationId: inv.id },
      });
      throw new IdentityError(
    ...
    if (!identityId) {
      ...
      if (existing) {
        ...
        if (!params.password) {
          throw new IdentityError(
            "Password required to create identity",
            400,
            "PASSWORD_REQUIRED"
          );
        }
        const created = await identityService.createWithPassword({
          email: inv.email,
    ...
    const roleKey =
      roles.find((r) => r.id === inv.roleId)?.key ?? "member";

    await membershipService.createMembership({
      organizationId: inv.organizationId,
      identityId,
      roleKey,
      actorIdentityId: inv.invitedById,
    });
    ...
    await emitIdentityEvent({
      eventType: IDENTITY_EVENTS.InvitationAccepted,
      identityId,
      organizationId: inv.organizationId,
      payload: { invitationId: inv.id },
    });
  ...
  async decline(params: { token: string }): Promise<void> {
    const inv = await invitationRepository.findByTokenHash(
