...
import { requireActorUserId, requirePositiveIntId, requireUploadEntityInOrg, resourceAuthErrorResponse } from "@/lib/auth/resource-authorization";
...
export async function GET(req: Request) {
  try {
    ...
    const runs = await prisma.ai_contract_resolution_runs.findMany({ where: { organization_id: orgId }, orderBy: { created_at: "desc" }, include: { _count: { select: { ai_contract_resolution_links: true } } } });
...
export async function POST(req: Request) {
  try {
    ...
    if (action === "extract") {
      ...
      const run = await prisma.ai_contract_resolution_runs.create({ data: { organization_id: orgId, user_id: userId, contract_hash, extractor_version: extractor_version || "v1", linker_version: linker_version || "v1" } });
    ...
    if (action === "resolve") {
      ...
      const normalizedLinks = await Promise.all((Array.isArray(body.links) ? body.links : []).map(async (link: any) => {
        ...
        if (!entityType) return NextResponse.json({ error: "entity_type is required", code: "VALIDATION_ERROR" }, { status: 400 });
        if (link?.entity_id === undefined || link?.entity_id === null || link?.entity_id === "") {
          return { entity_type: entityType, entity_id: null, action: link.action, confidence: link.confidence, rationale: link.rationale || null };
        }
        const entity = await requireUploadEntityInOrg(entityType, String(link.entity_id), ctx);
        return { entity_type: entity.entityType, entity_id: Number(entity.entityId), action: link.action, confidence: link.confidence, rationale: link.rationale || null };
      }));

      if (normalizedLinks.some((item) => item instanceof NextResponse)) return normalizedLinks.find((item) => item instanceof NextResponse) as NextResponse;
      const created = await Promise.all((normalizedLinks as any[]).map((link) => prisma.ai_contract_resolution_links.create({ data: { run_id: runId, entity_type: link.entity_type, entity_id: link.entity_id, action: link.action, confidence: link.confidence ?? null, rationale: link.rationale || null } })));
      return NextResponse.json(created, { status: 201 });
    }

    if (action === "link_suggest") {
      const body = await req.json();
      const keywords = (body.contract_hash || "").replace(/[_-]/g, " ").split(/\s+/).filter(Boolean);
