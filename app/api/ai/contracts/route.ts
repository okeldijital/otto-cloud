...
import { requireActorUserId, requirePositiveIntId, ResourceAuthError, resourceAuthErrorResponse } from "@/lib/auth/resource-authorization";
...
export async function GET(req: Request) {
  try {
    ...
    if (action === "extract") { const id = searchParams.get("id"); if (!id) return NextResponse.json({ error: "Missing id" }, { status: 400 }); const run = await prisma.ai_contract_resolution_runs.findFirst({ where: { id: requirePositiveIntId(id), organization_id: orgId }, include: { ai_contract_resolution_links: true, contract_intake_release_links: true } }); if (!run) return NextResponse.json({ error: "Run not found" }, { status: 404 }); return NextResponse.json(run); }
...
export async function POST(req: Request) {
  try {
    ...
    if (action === "extract") { const body = await req.json(); const run = await prisma.ai_contract_resolution_runs.create({ data: { organization_id: orgId, user_id: userId, contract_hash: body.contract_hash, extractor_version: body.extractor_version || "v1", linker_version: body.linker_version || "v1" } }); return NextResponse.json(run, { status: 201 }); }
    if (action === "resolve") {
      const body = await req.json(); const runId = requirePositiveIntId(body.run_id, "run_id"); const existingRun = await prisma.ai_contract_resolution_runs.findFirst({ where: { id: runId, organization_id: orgId } }); if (!existingRun) return NextResponse.json({ error: "Run not found" }, { status: 404 });
      const normalizedLinks = await Promise.all((Array.isArray(body.links) ? body.links : []).map(async (link: any) => {
        const entityType = String(link?.entity_type ?? "").trim(); if (!entityType) throw new ResourceAuthError("entity_type is required", 400, "VALIDATION_ERROR");
