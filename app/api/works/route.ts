...
type WorkContributorInput = { partyType: string; partyEntityId?: string | null; name: string; role: string; sharePercent?: number | null; controlledSharePercent?: number | null };
...
async function getWorkAggregate(workId: number, organizationId: string) {
  ...
  const [contributors, publishers, proRegistrations, recordings] = await Promise.all([
    prisma.$queryRaw<any[]>(Prisma.sql`SELECT * FROM public.work_contributors WHERE work_id = ${workId} AND organization_id = ${org} ORDER BY created_at ASC`),
    prisma.$queryRaw<any[]>(Prisma.sql`SELECT wp.*, p.name AS publisher_name, p.publisher_id AS publisher_external_id FROM public.work_publishers wp JOIN public.publishers p ON p.id = wp.publisher_id WHERE wp.work_id = ${workId} AND wp.organization_id = ${org} ORDER BY wp.created_at ASC`),
    prisma.$queryRaw<any[]>(Prisma.sql`SELECT wr.*, p.name AS pro_name, p.pro_id AS pro_external_id FROM public.work_pro_registrations wr JOIN public.pros p ON p.id = wr.pro_id WHERE wr.work_id = ${workId} AND wr.organization_id = ${org} ORDER BY wr.created_at ASC`),
    prisma.$queryRaw<any[]>(Prisma.sql`SELECT t.* FROM public.tracks t WHERE t.work_id = ${workId} AND (t.tenant_id = ${org} OR t.tenant_id IS NULL) ORDER BY t.created_at ASC`),
  ]);
  return { ...workRows[0], contributors, publishers, proRegistrations, recordings };
}

async function replaceAggregateRelations(tx: Prisma.TransactionClient, workId: number, organizationId: string, body: WorkAggregateInput) {
  const org = uuid(organizationId);
...
export async function GET(req: Request) {
  try {
    ...
    if (idStr) {
      ...
      const work = await getWorkAggregate(id, ctx.organizationId);
    ...
    const works = await prisma.$queryRaw<any[]>(Prisma.sql`SELECT w.*, COUNT(t.id)::int AS recording_count FROM public.works w LEFT JOIN public.tracks t ON t.work_id = w.id WHERE w.organization_id = ${org} AND w.is_deleted = false GROUP BY w.id ORDER BY w.created_at DESC OFFSET ${skip} LIMIT ${limit}`);
...
export async function DELETE(req: Request) {
  try {
    ...
    const org = uuid(ctx.organizationId);
    await prisma.$transaction(async (tx) => {
      await tx.$executeRaw(Prisma.sql`UPDATE public.tracks SET work_id = NULL, updated_at = CURRENT_TIMESTAMP WHERE work_id = ${id} AND (tenant_id = ${org} OR tenant_id IS NULL)`);
