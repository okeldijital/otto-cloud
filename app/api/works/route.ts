...
async function getWorkAggregate(workId: number, organizationId: string) {
  ...
  const [contributors, publishers, proRegistrations, recordings] = await Promise.all([
    prisma.$queryRaw<any[]>(Prisma.sql`SELECT wc.*, p.name AS pro_name, p.pro_id AS pro_external_id, pub.name AS publisher_name, pub.publisher_id AS publisher_external_id FROM public.work_contributors wc LEFT JOIN public.pros p ON p.id = wc.pro_id LEFT JOIN public.publishers pub ON pub.id = wc.publisher_id WHERE wc.work_id = ${workId} AND wc.organization_id = ${org} ORDER BY wc.created_at ASC`),
...
export async function DELETE(req: Request) {
  try {
    ...
    const org = uuid(ctx.organizationId);
    await prisma.$transaction(async (tx) => {
      await tx.$executeRaw(Prisma.sql`UPDATE public.tracks SET work_id = NULL, updated_at = CURRENT_TIMESTAMP WHERE work_id = ${id} AND (tenant_id = ${ctx.tenantId ? uuid(ctx.tenantId) : Prisma.sql`NULL`} OR tenant_id IS NULL)`);
