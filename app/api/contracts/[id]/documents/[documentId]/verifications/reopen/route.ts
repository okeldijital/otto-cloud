...
export async function POST(
  ...
  try {
    ...
    const verification = await verificationService.reopen({
      ctx,
      organizationId: ctx.organizationId,
    ...
    return ok({ verification }, "Verification reopened");
  } catch (error) {
    ...
    console.error("[POST reopen verification]", error);
    return fail("Unable to reopen verification", 500, "INTERNAL_ERROR");
  }
}
