...
const LABEL_COLUMNS = Prisma.raw(`
  id,
  label_id,
  name,
  address,
...
export async function POST(req: Request) {
  try {
    ...
    `;
    if (duplicate.length) {
      return NextResponse.json({ error: `A label with the name '${body.name}' already exists.` }, { status: 409 });
    }

    const created = await prisma.$queryRaw<any[]>`
      INSERT INTO labels (
        label_id, name, address, contact_email, contact_phone,
        website, logo_url, contact_person, organization_id,
        created_at, updated_at
      )
      VALUES (
        ${body.labelId}, ${body.name}, ${body.address}, ${body.contactEmail}, ${body.contactPhone},
        ${body.website}, ${body.logoUrl}, ${body.contactPerson}, CAST(${ctx.organizationId} AS uuid),
    ...
    if (err?.code === "P2002" || err?.code === "23505") return NextResponse.json({ error: "A label with this name or ID already exists." }, { status: 409 });
    return NextResponse.json({ error: "Internal server error" }, { status: 500 });
  }
}
...
export async function PUT(req: Request) {
  try {
    ...
    const duplicate = await prisma.$queryRaw<Array<{ id: number }>>`
      ...
      LIMIT 1
    `;
    if (duplicate.length) return NextResponse.json({ error: `A label with the name '${body.name}' already exists.` }, { status: 409 });

    const updated = await prisma.$queryRaw<any[]>`
      UPDATE labels
      SET label_id = ${body.labelId}, name = ${body.name}, address = ${body.address},
          contact_email = ${body.contactEmail}, contact_phone = ${body.contactPhone},
          website = ${body.website}, logo_url = ${body.logoUrl},
          contact_person = ${body.contactPerson}, updated_at = CURRENT_TIMESTAMP
      WHERE id = ${id}
...
export async function DELETE(req: Request) {
  try {
    ...
    const linked = await prisma.$queryRaw<Array<{ count: bigint }>>`
      SELECT COUNT(*)::bigint AS count FROM artists
      WHERE label_id = ${id} AND organization_id = CAST(${ctx.organizationId} AS uuid) AND is_deleted = false
    `;
    const linkedReleases = await prisma.$queryRaw<Array<{ count: bigint }>>`
      SELECT COUNT(*)::bigint AS count FROM releases
      WHERE label_id = ${id} AND organization_id = CAST(${ctx.organizationId} AS uuid) AND is_deleted = false
    `;
    if (Number(linked[0]?.count ?? 0) + Number(linkedReleases[0]?.count ?? 0) > 0) {
      return NextResponse.json({ error: "Cannot delete label because it is associated with artists or releases." }, { status: 409 });
    }
    ...
    if (err?.code === "P2003" || err?.code === "P2014" || err?.code === "23503") return NextResponse.json({ error: "Cannot delete label because it is associated with artists or releases." }, { status: 409 });
    return NextResponse.json({ error: "Internal server error" }, { status: 500 });
  }
}
