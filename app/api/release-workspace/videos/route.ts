...
export async function GET(req: Request) {
  try {
    ...
    console.error("[GET /api/release-workspace/videos]", err);
...
export async function POST(req: Request) {
  try {
    ...
    await prisma.workspace_timeline_events.create({
      data: { workspace_id: parsed.data.workspace_id, user_id: userId, event_type: "milestone", summary: `Video added: "${item.title}" (${item.video_type})` },
    });

    return NextResponse.json(item, { status: 201 });
  } catch (err: any) {
    console.error("[POST /api/release-workspace/videos]", err);
    return NextResponse.json({ error: "Internal server error" }, { status: 500 });
  }
}
...
export async function PUT(req: Request) {
  try {
    ...
    const existing = await requireWorkspaceVideoInOrg(parseInt(id), ctxMut);
    ...
    const updated = await prisma.workspace_videos.update({ where: { id: parseInt(id) }, data: updateData });
    ...
    console.error("[PUT /api/release-workspace/videos]", err);
    return NextResponse.json({ error: "Internal server error" }, { status: 500 });
  }
}
...
export async function DELETE(req: Request) {
  try {
    ...
    const existing = await requireWorkspaceVideoInOrg(parseInt(id), ctxMut);
    if (!existing) return NextResponse.json({ error: "Not found" }, { status: 404 });
    await prisma.workspace_videos.update({ where: { id: parseInt(id) }, data: { is_deleted: true } });
    ...
    console.error("[DELETE /api/release-workspace/videos]", err);
    return NextResponse.json({ error: "Internal server error" }, { status: 500 });
  }
}
