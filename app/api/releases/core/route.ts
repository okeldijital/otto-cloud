...
export async function GET(req: Request) {
  try {
    ...
    const [documents, financials, media, artistRoles, contract] = await Promise.all([
      prisma.$queryRaw<any[]>`SELECT rd.id, rd.file_name, rd.original_name, rd.mime_type, rd.file_size, rd.category, rd.description, rd.created_at, a.id AS attachment_id FROM release_documents rd LEFT JOIN attachments a ON a.organization_id = rd.organization_id AND a.entity_type = 'release' AND a.entity_id = CAST(rd.release_id AS TEXT) AND a.storage_key = rd.storage_key AND a.purpose = 'attachment' WHERE rd.organization_id = ${ctx.organizationId}::uuid AND rd.release_id = ${releaseId} ORDER BY rd.created_at DESC`,
