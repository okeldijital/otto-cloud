...
const COMPLETENESS_VARIANTS: Record<string, string> = {
  GREEN: "success",
  AMBER: "warn",
  RED: "critical",
};
...
export default function ContractsPage() {
  ...
  const isExpiredSoon = (endDate: string | null, days: number) => {
  ...
  const filtered = useMemo(() => {
    ...
    return contracts.filter((c) => {
      ...
      const matchesType =
        typeFilter === "All" ||
        (c.type || c.contract_type || "").toLowerCase() === typeFilter.toLowerCase();
      const matchesExpiring =
        expiring === 0 ||
        (c.end_date &&
          isExpiredSoon(c.end_date, expiring));
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        actions={
          <Button variant="orange" size="sm" onClick={() => setShowWizard(true)}>
          ...
          <div className="p-12 text-center text-text-secondary">
            {contracts.length === 0 ? (
              <div className="space-y-4">
                <h3 className="text-lg font-semibold text-white">Upload a signed contract PDF to begin.</h3>
                <p className="text-sm">OTTO does not create contracts — it organizes them.</p>
                <Button variant="orange" size="sm" onClick={() => setShowWizard(true)}>
            ...
            <table className="w-full" style={{ borderCollapse: "collapse" }}>
              <thead>
                <tr className="text-left text-xs uppercase tracking-wider text-text-secondary border-b border-white/5">
                  ...
                  <th className="p-4 font-bold">Parties</th>
                  <th className="p-4 font-bold">Assets</th>
              ...
              <tbody>
                {filtered.map((c) => {
                  const completeness = c.completeness || { score: 0, status: "RED", missing: [] };
                  const partyCount = c._count?.parties ?? c.contract_parties?.length ?? 0;
                  const docCount = c._count?.documents ?? c.contract_documents?.length ?? 0;
                  const endsSoon = c.end_date && isExpiredSoon(c.end_date, 30);
                  return (
                    <tr
                      key={c.id}
                    ...
                    >
                      ...
                      <td className="p-4">
                        <div className="font-medium text-white">{c.title || "Untitled contract"}</div>
                        <div className="text-xs text-text-secondary font-mono mt-0.5">{c.contract_number || "—"}</div>
                      </td>
                      <td className="p-4 text-sm text-text-secondary">
                        {partyCount > 0 ? (
                          <div className="flex flex-col gap-0.5">
                            {(c.contract_parties || []).slice(0, 2).map((p: any, idx: number) => (
                              <span key={idx} className="truncate max-w-[180px]">
                                {p.external_name || `${p.entity_type || "Party"} #${p.entity_id || ""}`}
                              </span>
                            ))}
                            {partyCount > 2 && (
                              <span className="text-xs text-text-secondary">+{partyCount - 2} more</span>
                            )}
                          </div>
                        ) : (
                          <span className="text-text-secondary/50">{partyCount} parties</span>
                        )}
                      </td>
                      <td className="p-4 text-sm text-text-secondary">
                        {c._count?.assets ?? c.contract_assets?.length ?? 0} tracks
                      </td>
                      <td className="p-4">
                        <div className="flex items-center gap-2">
                          <FileText size={14} className="text-text-secondary" />
                          <span className="text-sm">{docCount > 0 ? `v${docCount}` : "—"}</span>
                        </div>
                        <div className="mt-1">
                          <Badge
                            variant={COMPLETENESS_VARIANTS[completeness.status] || "neutral"}
                            size="sm"
                          >
                            {completeness.status}
                          </Badge>
                      ...
                      <td className={`p-4 text-sm ${endsSoon ? "text-danger" : "text-text-secondary"}`}>
                      ...
                      <td className="p-4">
                        <div className="flex gap-2">
                          ...
                          <Button
                            ...
                            onClick={(e: React.MouseEvent) => {
                              e.stopPropagation();
                              router.push(`/contracts/${c.id}?tab=parties`);
                            }}
                          >
                            Add Parties
                          </Button>
      ...
      <AddContractWizard
        ...
        onCreated={(created: any) => {
          const cid = created?.id || created?.contract_id;
