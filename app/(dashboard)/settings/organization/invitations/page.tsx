...
export default function OrgInvitationsPage() {
  ...
  const load = async () => {
    ...
    const availableRoles = (rolesData.roles || []).filter((r: Role) => r.key !== "owner" && r.key !== "org_admin");
    setRoles(availableRoles);
    if (availableRoles.length && !availableRoles.some((r: Role) => r.key === roleKey)) setRoleKey(availableRoles[0].key);
  };
  ...
  const invite = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/auth/invitations", { method: "POST", credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ email, roleKey }) });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) { setError(data.error || "Invitation failed"); return; }
      setMessage(`Invitation sent to ${email}.`); setEmail(""); await load();
    } finally { setSending(false); }
  };

  const cancel = async (id: string) => {
    const res = await fetch(`/api/auth/invitations?invitation_id=${encodeURIComponent(id)}`, { method: "DELETE", credentials: "include" });
    const data = await res.json().catch(() => ({}));
    if (!res.ok) setError(data.error || "Unable to cancel invitation"); else await load();
  };
  ...
  return (
    <div className="p-8 max-w-3xl mx-auto space-y-6 text-white">
      ...
      {error && <p className="text-sm text-danger">{error}</p>}{message && <p className="text-sm text-green-400">{message}</p>}
      <ul className="space-y-2 text-sm">{invitations.map((i) => <li key={i.id} className="flex justify-between p-3 rounded-lg bg-white/5 border border-white/10"><div>{i.email} · {i.roleKey || "member"} · {i.status}<div className="text-xs text-white/40">expires {new Date(i.expiresAt).toLocaleString()}</div></div>{i.status === "pending" && <button className="text-xs text-danger underline" onClick={() => cancel(i.id)}>Cancel</button>}</li>)}</ul>
    </div>
  );
}
