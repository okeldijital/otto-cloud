...
export default function OrganizationIamSettingsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {tab === "team" && <Card title={`Team Members (${members.length})`}><div className="flex justify-end mb-4"><Link href="/settings/organization/invitations" className="px-4 py-2 rounded-lg bg-accent text-white text-sm font-semibold">Manage invitations</Link></div>{members.length === 0 ? <p className="text-sm text-text-secondary">No members returned by the canonical IAM membership service.</p> : <div className="space-y-2">{members.map((member) => <div key={member.identityId} className="flex items-center justify-between gap-4 p-4 rounded-lg bg-white/5 border border-white/10"><button type="button" onClick={() => setSelectedMember(member)} className="min-w-0 text-left rounded-md focus:outline-none focus:ring-2 focus:ring-accent/50"><div className="font-medium text-white hover:text-accent transition-colors">{member.displayName || member.email || member.identityId}</div><div className="text-xs text-text-secondary">{member.email || "—"} · {member.membershipStatus}</div></button><Badge variant="primary" size="sm">{member.isOwner ? "Owner" : member.roleName || member.role || "member"}</Badge></div>)}</div>}
      {selectedMember && <div className="mt-4 p-4 rounded-lg bg-surface-elevated border border-white/10"><div className="flex items-start justify-between gap-4"><div><p className="text-sm font-semibold text-white">Member details</p><p className="text-xs text-text-secondary mt-1">{selectedMember.isOwner ? "Organization owner" : "Organization member"}</p></div><button type="button" onClick={() => setSelectedMember(null)} className="text-xs text-text-secondary hover:text-white">Close</button></div><div className="grid grid-cols-1 md:grid-cols-2 gap-2 mt-4"><Row label="Name" value={selectedMember.displayName || "—"} /><Row label="Email" value={selectedMember.email || "—"} /><Row label="Role" value={selectedMember.isOwner ? "Owner" : selectedMember.roleName || selectedMember.role || "member"} /><Row label="Membership" value={selectedMember.membershipStatus} /><Row label="Identity ID" value={selectedMember.identityId} mono /></div></div>}
      </Card>}
