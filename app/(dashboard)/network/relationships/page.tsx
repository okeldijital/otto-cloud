...
import { Share2, ArrowRight, Calendar, Info } from "lucide-react";
      ...
      <PageHeader
        ...
        actions={
          <Button variant="primary" size="sm">
            <Share2 size={14} /> Define Relationship
          </Button>
        }
      />
      ...
      {relationships.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {relationships.map((rel) => (
            <div key={rel.id} className="bg-premium-glass border border-white/5 rounded-2xl p-6 backdrop-blur-xl hover:border-white/20 transition-all">
              ...
              <h3 className="text-lg font-bold text-white mb-2">
                <span className="text-sm font-normal text-text-secondary uppercase tracking-widest">{rel.relationship_type?.replace(/_/g, " ")}</span>
              </h3>
              ...
              <div className="bg-white/5 rounded-xl p-3 text-sm text-text-secondary italic mb-4">
                "{rel.notes || "No notes provided for this relationship."}"
              </div>

              <div className="flex justify-between items-center">
                <button className="text-xs font-bold text-text-secondary hover:text-white uppercase tracking-widest transition-colors">
                  View Contract
                </button>
                <Badge variant="success" size="sm">Governed</Badge>
              </div>
            </div>
          ))}
        </div>
        ...
        <Card>
          <div className="py-12 text-center">
            ...
            <h3 className="text-xl font-bold text-white mb-2">No relationships defined yet</h3>
            ...
            <div className="flex flex-wrap justify-center gap-3 text-xs">
              <span className="px-3 py-1 bg-white/5 rounded-full border border-white/10 text-text-secondary">
                Artist signed TO Publisher
              </span>
              <span className="px-3 py-1 bg-white/5 rounded-full border border-white/10 text-text-secondary">
                Work registered WITH PRO
              </span>
              <span className="px-3 py-1 bg-white/5 rounded-full border border-white/10 text-text-secondary">
                Artist released VIA Distributor
              </span>
            </div>
          </div>
        </Card>
