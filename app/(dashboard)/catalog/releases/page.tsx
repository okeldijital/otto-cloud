  ...
  const columns = useMemo(() => [
    {
      key: "artwork",
      label: "",
      render: (row: any) => <EntityArtwork entityType="release" entityId={row.id} src={coverUrls[String(row.id)] ?? null} alt={row.title} size={40} placeholder="release" className="rounded-lg" style={{ borderRadius: 8 }} />,
    },
    { key: "title", label: "Title", sortable: true },
    { key: "release_type", label: "Type", sortable: true },
    { key: "release_date", label: "Release Date", sortable: true, render: (row: any) => row.release_date ? new Date(row.release_date).toLocaleDateString() : "—" },
    { key: "catalog_number", label: "Catalog #", render: (row: any) => row.catalog_number || "—" },
  ], [coverUrls]);
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        actions={<Button variant="primary" size="sm" onClick={() => setShowAddModal(true)}><Plus size={16} />Add Release</Button>}
      />
      ...
      <section className="flex flex-col gap-3 border-b border-border pb-4 md:flex-row md:items-center md:justify-between" aria-label="Release catalogue controls">
        <div className="flex w-full flex-col gap-3 sm:flex-row">
          <div className="relative w-full sm:max-w-md">
            ...
            <input type="search" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search releases..." aria-label="Search releases" className="h-10 w-full rounded-md border border-border bg-surface pl-9 pr-3 text-sm text-text-primary placeholder:text-text-secondary/70 outline-none transition-colors focus:border-accent focus:ring-1 focus:ring-accent/50" />
          </div>
          <label className="relative flex h-10 items-center rounded-md border border-border bg-surface px-3 text-sm text-text-secondary">
            ...
            <select value={typeFilter} onChange={(e) => setTypeFilter(e.target.value)} aria-label="Filter releases by type" className="appearance-none bg-transparent pr-5 text-sm text-text-primary outline-none">
              <option value="all">All types</option><option value="single">Single</option><option value="ep">EP</option><option value="album">Album</option><option value="compilation">Compilation</option>
            </select>
          </label>
        </div>
      </section>

      <DataTable columns={columns} data={filteredData} isLoading={loading} onRowClick={(row: any) => router.push(`/catalog/releases/${row.id}`)} onEdit={(row: any) => router.push(`/catalog/releases/${row.id}`)} onDelete={handleDelete} />
