  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <button onClick={() => router.push("/catalog/publishers")} className="text-text-secondary hover:text-text-primary transition-colors" aria-label="Back to publishers">
          <ArrowLeft size={20} />
        </button>
        <PageHeader
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card title="Identity & Contact">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div><span className="text-text-secondary text-xs block">Name</span><span className="font-medium">{publisher.name || "—"}</span></div>
              <div><span className="text-text-secondary text-xs block">Publisher ID</span><span className="flex items-center gap-1"><Hash size={14} />{publisher.publisher_id || "—"}</span></div>
              <div><span className="text-text-secondary text-xs block">Contact Person</span><span className="flex items-center gap-1"><User size={14} />{publisher.contact_person || "—"}</span></div>
              <div><span className="text-text-secondary text-xs block">Email</span><span className="flex items-center gap-1"><Mail size={14} />{publisher.contact_email || "—"}</span></div>
              <div><span className="text-text-secondary text-xs block">Phone</span><span className="flex items-center gap-1"><Phone size={14} />{publisher.contact_phone || "—"}</span></div>
              <div><span className="text-text-secondary text-xs block">Website</span><span className="flex items-center gap-1"><Globe size={14} />{publisher.website || "—"}</span></div>
              <div><span className="text-text-secondary text-xs block">Rights Type</span><span>{publisher.rights_type || "—"}</span></div>
              <div className="md:col-span-2"><span className="text-text-secondary text-xs block">Address</span><span className="flex items-center gap-1"><MapPin size={14} />{publisher.address || "—"}</span></div>
            </div>
          </Card>
            ...
            {artists.length === 0 ? (
              ...
              <div className="space-y-2">
                {artists.map((artist: any) => (
                  <div key={artist.id} className="flex items-center gap-3 p-2 rounded-lg bg-surface-elevated cursor-pointer hover:bg-surface" onClick={() => router.push(`/catalog/artists/${artist.id}`)}>
                    <Music size={16} /><span>{artist.name}</span>
                  </div>
                ))}
              </div>
            )}
        ...
        <div className="space-y-6">
          <Card title="Linked Works">
            {works.length === 0 ? (
              ...
              <div className="space-y-2">
                {works.map((work: any) => (
                  <div key={work.id} className="flex items-center justify-between p-2 rounded-lg bg-surface-elevated cursor-pointer hover:bg-surface" onClick={() => router.push(`/catalog/works/${work.id}`)}>
        ...
        <div className="space-y-8">
          <section>
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div className="sm:col-span-2"><label className="mb-1.5 block text-xs font-medium text-text-secondary">Publisher Name *</label><input className={inputClass} value={editData.name || ""} onChange={(e) => setEditData({ ...editData, name: e.target.value })} required /></div>
              <div><label className="mb-1.5 block text-xs font-medium text-text-secondary">Publisher ID</label><input className={inputClass} value={editData.publisher_id || ""} onChange={(e) => setEditData({ ...editData, publisher_id: e.target.value })} /></div>
              <div><label className="mb-1.5 block text-xs font-medium text-text-secondary">Rights Type</label><input className={inputClass} value={editData.rights_type || ""} onChange={(e) => setEditData({ ...editData, rights_type: e.target.value })} /></div>
            </div>
          </section>
          <section>
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              <div><label className="mb-1.5 block text-xs font-medium text-text-secondary">Website</label><input type="url" className={inputClass} value={editData.website || ""} onChange={(e) => setEditData({ ...editData, website: e.target.value })} placeholder="https://..." /></div>
